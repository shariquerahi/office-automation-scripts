...
output_dir = '/Users/shariquerahi/Desktop/Git/Python_Script/output_BMS_DATA.CSV'
...
small_file.to_csv(os.path.join(output_dir, 'small_file_updated.csv'), index=False)
...
# Convert 'Authorization_amount' columns to numeric
result['Authorization_amount_x'] = pd.to_numeric(result['Authorization_amount_x'], errors='coerce')
result['Authorization_amount_y'] = pd.to_numeric(result['Authorization_amount_y'], errors='coerce')
...
result = result[
    (result['Authorization_amount_x'].notna() & result['Authorization_amount_y'].notna() &
     (abs(result['Authorization_amount_x'] - result['Authorization_amount_y']) <= amount_tolerance)) &
    (abs(result['transaction_date_x'] - result['transaction_date_y']) <= date_tolerance)
]

# Saving the matching rows to a new CSV file
result.to_csv(os.path.join(output_dir, 'matching_rows.csv'), index=False)
...
unmatched_result.to_csv(os.path.join(output_dir, 'unmatched_rows.csv'), index=False)

# Group by the month and count the number of transactions in each month
monthly_count_big = big_file.groupby(big_file['transaction_date'].dt.strftime('%Y-%m'))['transaction_date'].count()
monthly_count_small = small_file.groupby(small_file['transaction_date'].dt.strftime('%Y-%m'))['transaction_date'].count()

# Print the result
print('Monthly Count (big_file):', monthly_count_big)
print('Monthly Count (small_file):', monthly_count_small)
...
unmatched_result = small_file.merge(
    result, left_on=['Card_last_4_digit', 'transaction_date','Authorization_amount'], 
    right_on=['Card_last_4_digit', 'transaction_date_x','Authorization_amount_x'], how='left', indicator=True
).loc[lambda x: x['_merge'] == 'left_only']

# Save the unmatched rows to a new CSV file
unmatched_result.to_csv(os.path.join(output_dir, 'unmatched_rows_final.csv'), index=False)

# Print the number of unmatched rows
print("Number of unmatched rows (final):", len(unmatched_result))
