...
def append_to_results(ticket, actor_id, dbinfo):
    if dbinfo:
    ...
    row = [ticket, actor_id, card_id, actor_id, state, card_form, bank_identifier, masked_card_number]
...
df = pd.DataFrame(card_results, columns=['ticket', 'actor_id', 'cardId', 'actor_id','state','card_form','bank_identifier','masked_card_number'])
