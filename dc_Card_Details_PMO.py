...
def append_to_results(ticket, actor_id, status, dbinfo):
    ...
    row = [ticket, actor_id, status, card_id,state1, card_form, bank_identifier, masked_card_number,requestId, state]
    results.append(row)
...
df = pd.DataFrame(results, columns=['ticket_id', 'actor_id', 'status', 'card_id', 'state1','card_form', 'bank_identifier', 'masked_card_number','requestId', 'state'])
