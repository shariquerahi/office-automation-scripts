...
def getCardCreationRequest(ref_id):
...
# Read the CSV data with 'ref_id' and 'ticket' columns
data = pd.read_csv(csv_input_path)

card_results = []

def append_to_results(ref_id, dbinfo):
    if dbinfo:
        actor_id = dbinfo.get('actor_id')
    else:
        actor_id= 'N/A'
...
try:
    for index, row in data.iterrows():
        ref_id = row['ref_id']
        try:
            print("Attempting to get card creation request details for ref_id:", ref_id)
            dbinfo = getCardCreationRequest(ref_id)
            #print(f"API Response for {ref_id}: {dbinfo}")
            append_to_results(ref_id, dbinfo)
        except Exception as e:
            print("Exception when processing card ID:", ref_id, e)
            append_to_results(ref_id, None)
        time.sleep(3)
        count += 1
    print("Completed")
except Exception as e:
...
df = pd.DataFrame(card_results, columns=['ref_id', 'actor_id'])
