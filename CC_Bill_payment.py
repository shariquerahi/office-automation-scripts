...
def getCardCreationRequest(order_id):
    ...
    params = {
        'service': 'FIREFLY',
        'entity': 'CREDIT_CARD_BILL_PAYMENT',
        'options': json_dump,
        'monorailId': '1',
    }
    try:
...
'''def getCardCreationRequest1(credit_account_id):
    url = "https://sherlock.epifi.in/api/v1/db-states/info"
    opts = [
        {
...
csv_output_path = "/Users/shariquerahi/Desktop/Git/Python_Script/Output_Firefly.csv"
...
def append_to_results(order_id, dbinfo):
    try:
        if isinstance(dbinfo, dict) and dbinfo:  # Check if dbinfo is a non-empty dictionary
            amount = dbinfo.get('amount', 'N/A')
            external_txn_id = dbinfo.get('external_txn_id', 'N/A')
            payment_date = dbinfo.get('payment_date', 'N/A')
            account_id=dbinfo.get('account_id', 'N/A')

        else:
            amount, external_txn_id, payment_date,account_id = 'N/A', 'N/A', 'N/A','N/A'
    except Exception as e:
        print(f"Error while processing API response for order_id: {order_id}. Error: {e}")
        amount, external_txn_id, payment_date,account_id = 'N/A', 'N/A', 'N/A','N/A'


    row = [order_id, amount, external_txn_id, payment_date,account_id]
...
# Add the following print external_txn_idment to check the DataFrame before saving to CSV
# ... (previous code remains the same)

# Add the following print statement to check the DataFrame before saving to CSV
df = pd.DataFrame(card_results, columns=['order_id', 'amount', 'external_txn_id', 'payment_date','account_id'])
